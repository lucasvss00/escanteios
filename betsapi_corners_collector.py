...
def parse_stats_trend(raw_trend, event_id: str, meta: dict,
    ...
    collected_ts = datetime.now(UTC).isoformat()
...
def run_historico(
    ...
    print(f"\n{'═'*62}")
    print(f"  BetsAPI Corner Collector  —  {datetime.now(UTC).strftime('%Y-%m-%d %H:%M')} UTC")
    print(f"{'─'*62}")
    ...
    try:
        while True:
            if max_iterations is not None and iteration >= max_iterations:
                break
            iteration += 1
            log.info("--- Ciclo %d | %s ---", iteration, datetime.now(UTC).isoformat())
